...
{if isset($p) AND $p}
			...
			<form class="showall" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get">
				<div>
	                ...
	                <button type="submit" class="btn btn-default button exclusive-medium">
	                	<span>{l s='Show all' d='Admin.Actions'}</span>
	                </button>
				...
				{if $p != 1}
					...
					<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous">
						<a{$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}" rel="prev">
							<i class="icon-chevron-left"></i> <b>{l s='Previous' d='Admin.Actions'}</b>
						</a>
					</li>
				{else}
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next">
						<a{$no_follow_text} href="{$link->goPage($requestPage, $p_next)}" rel="next">
							<b>{l s='Next' d='Admin.Actions'}</b> <i class="icon-chevron-right"></i>
						</a>
					</li>
				{else}
    ...
    <div class="product-count">
        ...
        {/if}
        {if $nb_products > 1}
        	{l s='Showing %1$d - %2$d of %3$d items' sprintf=[$productShowingStart, $productShowing, $nb_products] d='Modules.Omnivaltshipping.Admin'}
		{else}
        	{l s='Showing %1$d - %2$d of 1 item' sprintf=[$productShowingStart, $productShowing] d='Modules.Omnivaltshipping.Admin'}
       	{/if}
    </div>
	<!-- /Pagination -->
{/if}
