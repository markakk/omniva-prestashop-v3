    ...
    $.fn.omniva = function(options) {
        ...
        */
        ...
        function initMap(){
          ...
          $('#omniva-search .omniva-search-form input').off('keypress').on('keypress',function(e){
                if (e.which === 13) {
                    e.preventDefault();
                }
            });
          $('#omniva-search .omniva-search-form input').off('keyup focus').on('keyup focus',function(e){
                if (e.type === 'keyup' && e.which === 13) {
                    e.preventDefault();
                    $('#omniva-search .omniva-search-form #map-search-button').trigger('click');
                    return;
                }
            ...
            $('#omniva-search .omniva-search-form #map-search-button').off('click').on('click',function(e){
              e.preventDefault();
...
function launch_omniva(retry = 0) {
    ...
    if (retry >= 50) {
        console.log('OMNIVA:', 'Failed to load terminal selection after 50 retries.');
    ...
    if ($('#omnivalt_parcel_terminal_carrier_details select').length){
        $('.omniva-terminal-loading').hide();
        $('#omnivalt_parcel_terminal_carrier_details select').omniva({showMap: omnivalt_params.show_map});
